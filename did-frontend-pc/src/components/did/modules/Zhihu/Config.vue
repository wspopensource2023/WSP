<template>
    <div>
        <div>
            <ModuleConfigHeader :groupIndex="groupIndex" :moduleIndex="moduleIndex" title="知乎" @delete="deleteModule"
                description="展示你在知乎的账号，让访问者可以直接了解你在知乎上的影响力以及分享的知识。" icon="zhihu">
            </ModuleConfigHeader>
        </div>
        <div class="tw-pt-8 tw-px-3">
            <div>
                <v-text-field class="tw-mt-3" hide-details type="text" :persistent-placeholder="true" rounded="lg" label="知乎账号ID"
                    variant="outlined" v-model="config.spec.uid" placeholder="请输入知乎个人主页链接或账号UID，输入链接将自动识别" @input="onInputUID"  />
                <div class="tw-flex tw-flex-row tw-mb-3 tw-mt-3">
                    <div
                        @click="showVideoModal('如何获取知乎的个人主页链接', 'https://static.xxxxxx.com/aaa3609bfecacae63c62eb5c4bbc0b3450cac25037e8bed6fad372d8824aebcd')"
                        ...
                        <div class="tw-flex tw-items-center">
                            <img
                                ...
                                alt="">
                            <span
                                class="tw-ml-1">不知道如何获取知乎的个人主页链接? 点我查看帮助视频</span>
                        </div>
                    </div>
                    <a target="_black" href="https://www.zhihu.com" class="tw-flex tw-flex-row tw-opacity-50 hover:tw-opacity-100 tw-transition-all tw-duration-300 tw-cursor-pointer tw-bg-white/10 tw-rounded tw-px-3 tw-py-0.5 tw-items-center tw-ml-2">
                            <img class="tw-w-4 tw-h-4"
                                src="https://static.xxxxxx.com/c490e44a1408c1baff5e49144079334c8ed75d71ff93e005d8ebf7b3136cdb6a"
                                alt="">
                            <span
                                class="tw-ml-1">打开知乎</span>
                    </a>
                </div>
...
import {parseZhihuUID} from "@/library/util";
export default {
    props: {
        moduleIndex: -1,
        groupIndex: -1,
    ...
    methods: {
        ...
        onInputUID(val) {
            this.config.spec.uid = parseZhihuUID(this.config.spec.uid)
        },
        async aigc() {
            ...
            const res = await apiService.AIGCZhiHuCRD(uid);
