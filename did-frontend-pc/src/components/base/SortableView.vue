...
export default {
    ...
    methods: {
        ...
        touchstart(clientX, clientY, target) {
            ...
            this.draggingElementShadow = this.createShadowElement(
        ...
        touchend() {
            ...
            draggingElement.style.transform =
                this.draggingElementStyle.transform;
        ...
        prepare() {
            ...
            for (let i = 0; i < parents.length; i++) {
                ...
                const elements = [];
                const children = $(parent).find(this.itemSelector).get();
        ...
        getTargetShadowNodePosition(cursorX, cursorY) {
            ...
            const skipIndex = (() => {
                const draggingGroupId = this.getElementGroupId(
                    this.draggingElement
                );
                const draggingItemId = this.getElementItemId(
                    this.draggingElement
                );
        ...
        removeTargetElementShadow() {
            const $node = $(`.${this.targetShadowID}`);
            $node.slideUp(100, () => {
                $node.remove();
            });
        },
        ...
        createTargetShadowNode(cursorX, cursorY) {
            ...
            this.position = position;
            const { groupId, direction, index } = position;
            this.removeTargetElementShadow();
            targetElementShadow = this.createShadowElement(
                this.draggingElement
            );
            ...
            if (direction === 1) {
                ...
                if (elements.length === 0) {
                    ...
                    const rel = elements[index];
                    rel.parentNode.insertBefore(
                        targetElementShadow,
                        rel.nextSibling
                    );
                }
            }
            $(targetElementShadow).hide().stop().slideDown(100);
