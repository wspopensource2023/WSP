<template>
    <div>
        <uni-popup ref="modal">
            <div style="background: linear-gradient(180deg, #252525 0%, #181818 100%); height: 32rem"
                ...
                <div class="tw-pt-3 tw-pb-6 tw-px-1">
                    <div class="tw-flex tw-flex-row">
                        <SearchInput class="tw-mr-3" v-model="keywords" @input="search" @search="search"
                            placeholder="请输入关键词">
                        </SearchInput>
                        <button @click="triggerUpload" hover-class="none"
                            class="tw-w-28 tw-rounded-xl tw-bg-white tw-bg-opacity-5 hover:tw-bg-opacity-10 tw-transition">
                            <div
                    ...
                    <div class="tw-max-h-72 tw-overflow-y-auto tw-grid tw-grid-cols-6 tw-gap-y-1 tw-mt-3">
                        <div @click="select(icon.imageUri)" :class="getIconClass(icon.imageUri)" :key="icon.key"
