    ...
    <div>
        <div>
            <ModuleConfigHeader :groupIndex="groupIndex" :moduleIndex="moduleIndex" class="tw-pt-5" icon="xiaohongshu"
                title="小红书" description="展示你在小红书的账号，让访问者可以直接了解你在小红书上的影响力以及分享的内容。" @delete="deleteModule">
            </ModuleConfigHeader>
        </div>
        <div class="tw-pt-3 tw-px-2">
            <div v-if="isTab('simple')">
                <TextInput class="tw-mt-3" label="UID" v-model="config.spec.uid" placeholder="请输入小红书账号UID">
                </TextInput>
                <div class="tw-mt-3">
...
export default {
    ...
    methods: {
        ...
        async aigc() {
            ...
            const res = await apiService.AIGCXiaoHongShuCRD(this.uid);
