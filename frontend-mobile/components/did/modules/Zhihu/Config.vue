<template>
    <div>
        <div>
            <ModuleConfigHeader :groupIndex="groupIndex" :moduleIndex="moduleIndex" class="tw-pt-5" icon="zhihu" title="知乎"
                description="展示你在知乎的账号，让访问者可以直接了解你在知乎上的影响力以及分享的知识。" @delete="deleteModule">
            </ModuleConfigHeader>
        </div>
        <div class="tw-pt-3 tw-px-2">
            <div v-if="isTab('simple')">
                <TextInput class="tw-mt-3" label="UID" v-model="config.spec.uid" placeholder="请输入知乎账号UID">
                </TextInput>
                <div class="tw-mt-3">
...
export default {
    ...
    methods: {
        ...
        async aigc() {
            ...
            const res = await apiService.AIGCZhiHuCRD(this.uid);
